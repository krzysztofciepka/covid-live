...
const updateState = (update) => {
    ...
    if (infectedUpdate) {
        state.changes.unshift(`${moment().format('MMMM Do YYYY, h:mm:ss a')}    ${infectedUpdate > 0 ? '+' : ''}${infectedUpdate} infected`);
    }

    if (recoveredUpdate) {
        state.changes.unshift(`${moment().format('MMMM Do YYYY, h:mm:ss a')}    ${recoveredUpdate > 0 ? '+' : ''}${recoveredUpdate} recovered`);
    }

    if (deathsUpdate) {
        state.changes.unshift(`${moment().format('MMMM Do YYYY, h:mm:ss a')}    ${deathsUpdate > 0 ? '+' : ''}${deathsUpdate} deaths`);
    }
